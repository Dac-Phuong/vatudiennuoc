...
// Khởi tạo và load cấu hình từ localStorage
const LAYOUT_CONFIG_KEY = 'vue-layout-config';

// Load saved config khi component mounted
onMounted(() => {
    const savedConfig = localStorage.getItem(LAYOUT_CONFIG_KEY);
    if (savedConfig) {
        const parsedConfig = JSON.parse(savedConfig);
        Object.assign(layoutConfig, parsedConfig);

        // Áp dụng các thay đổi từ config đã lưu
        preset.value = layoutConfig.preset;
        menuMode.value = layoutConfig.menuMode;

        // Cập nhật theme ngay lập tức
        applyTheme('primary', { name: layoutConfig.primary });
...
// Tự động lưu khi layoutConfig thay đổi
watch(layoutConfig, (newConfig) => {
    localStorage.setItem(LAYOUT_CONFIG_KEY, JSON.stringify(newConfig));
}, { deep: true });
    ...
    <div
        ...
        <div class="flex flex-col gap-4">
            ...
            <div class="flex flex-col gap-2">
                ...
                <Select v-model="preset" @change="onPresetChange" :options="presetOptions" :allowEmpty="false" />
            </div>
            <div class="flex flex-col gap-2">
                <span class="text-sm text-muted-color font-semibold">Menu Mode</span>
                <Select v-model="menuMode" @change="onMenuModeChange" :options="menuModeOptions" :allowEmpty="false"
