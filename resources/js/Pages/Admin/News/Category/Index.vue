...
const fetchCategories = debounce((page = 1) => {
    loading.value = true;
    router.get(route('admin.news.categories'), {
        search: search.value,
        page: page,
...
// call api
const create = () => {
    formCreate.post(route('admin.news.categories.store'), {
        onSuccess: () => {
            formCreate.reset();
...
const update = () => {
    formEdit.post(route('admin.news.categories.update', formEdit.id), {
        onSuccess: () => {
...
const deleteCategory = (data) => {
    confirm.require({
        ...
        accept: () => {
            router.delete(route('admin.news.categories.destroy', data.id), {
                onSuccess: () => {
...
<template>
    ...
    <Head title="Quản lý danh mục bài viết" />
    <AppLayout>
        <div class="card">
            <DataTable :value="data.data" dataKey="id" stripedRows responsiveLayout="scroll" :paginator="true"
                ...
                <template #header>
                    <div class="flex flex-wrap gap-2 mb-5 items-center justify-between">
                        <h2 class="m-0 text-2xl font-sans">Quản lý danh mục bài viết</h2>
                ...
                <Column field="name" header="Tiêu đề" style="min-width: 12rem">
                    <template #body="{ data }">
                        <span>{{ data.name }}</span>
                    </template>
                </Column>
                ...
                <Column field="is_show" header="Trạng thái" sortable>
                    <template #body="{ data }">
        ...
        <!-- modal create -->
        <Dialog v-model:visible="modal.create" modal header="Thêm danh mục bài viết" :style="{ width: '30rem' }">
