...
const submit = () => {
    ...
    if (!form.email || !form.password) {
        notify.error('Vui lòng nhập đầy đủ thông tin!');
        return;
    }
    ...
    form.post(route('admin.login'), {
        ...
        onSuccess: () => {
            notify.success('Đăng nhập thành công!');
        },
...
<template>
    ...
    <Head title="Đăng nhập" />
    <div
        class="bg-surface-50 dark:bg-surface-950 flex items-center justify-center min-h-screen min-w-[100vw] overflow-hidden">
        <div class="flex flex-col items-center justify-center">
            <div
                style="border-radius: 56px; padding: 0.3rem; background: linear-gradient(180deg, var(--primary-color) 10%, rgba(33, 150, 243, 0) 30%)">
                <div class="w-full bg-surface-0 dark:bg-surface-900 shadow-2xl py-20 px-8 sm:px-14"
                    ...
                    <div class="text-center mb-8">
                        ...
                        <div class="text-surface-900 dark:text-surface-0 text-3xl font-medium mb-4">Chào mừng
                            <br>
                            bạn đến với trang quản trị
                        </div>
                        <span class="text-muted-color font-medium">Đăng nhập để tiếp tục</span>
                    </div>

                    <form @submit.prevent="submit">
                        <label for="email1"
                            class="block text-surface-900 dark:text-surface-0 text-base font-medium mb-2">Địa chỉ
                            Email</label>
                        <InputText id="email1" type="text" placeholder="Email address" class="w-full md:w-[30rem] mb-8"
                            v-model="form.email" />

                        <label for="password1"
                            class="block text-surface-900 dark:text-surface-0 font-medium text-base mb-2">Mật
                            khẩu</label>
                        <InputText id="password1" type="password" placeholder="Password"
                            class="w-full md:w-[30rem] mb-8" v-model="form.password" />

                        <div class="flex items-center justify-between mt-2 mb-8 gap-8">
                            <div class="flex items-center">
                                <label class="flex items-center cursor-pointer">
                                    <Checkbox v-model="form.remember" binary class="mr-2" />
                                    <span>Lưu mật khẩu</span>
                                </label>
                            </div>

                        </div>
                        <Button label="Đăng Nhập" class="w-full" type="submit"></Button>
                    </form>
                </div>
            </div>
        </div>
